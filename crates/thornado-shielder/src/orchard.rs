...
pub fn create_orchard_note(
    ...
    builder
        ...
        .map_err(|e| Error::Shielder(format!("Orchard output build failed: {e:?}")))?;
    let (unauthorized, meta): (Bundle<_, i64>, _) = builder
        .build(&mut rng)
        .map_err(|e| Error::Shielder(format!("Orchard shielding bundle build failed: {e:?}")))?
...
pub fn prove_orchard_withdrawal(
    ...
    builder
        ...
        .map_err(|e| Error::Shielder(format!("Orchard spend build failed: {e:?}")))?;
    let (unauthorized, meta): (Bundle<_, i64>, _) = builder
        .build(&mut rng)
        .map_err(|e| Error::Shielder(format!("Orchard spend bundle build failed: {e:?}")))?
        .ok_or(Error::InvalidProof)?;
    ...
    let proven = unauthorized
        ...
        .map_err(|e| Error::Shielder(format!("Orchard spend proof failed: {e:?}")))?;
    let spend_bundle = proven
        .apply_signatures(&mut rng, sighash, &[SpendAuthorizingKey::from(&sk)])
        .map_err(|e| Error::Shielder(format!("Orchard spend signature failed: {e:?}")))?;
    verify_bundle(
        &spend_bundle,
        &vk,
        &[b"thornado:withdraw".as_slice(), public_context].concat(),
...
pub fn merkle_root_hex(leaves: &[String]) -> Result<String> {
    ...
    let root = tree
        ...
        .map_err(|e| Error::Shielder(format!("Orchard root lookup failed: {e:?}")))?
...
pub fn prove_orchard_spend_smoke(seed: &str, value_sats: u64) -> Result<Vec<u8>> {
    ...
    let shielding_bundle: Bundle<_, i64> = {
        ...
        let proven = unauthorized
            .create_proof(&pk, &mut rng)
            .map_err(|e| Error::Shielder(format!("Orchard shielding proof failed: {e:?}")))?;
        proven
            .apply_signatures(&mut rng, sighash, &[])
            .map_err(|e| Error::Shielder(format!("Orchard shielding signature failed: {e:?}")))?
    };
    ...
    let spend_bundle: Bundle<_, i64> = {
        ...
        .map_err(|e| Error::Shielder(format!("Orchard tree append failed: {e:?}")))?;
        let root = tree
        ...
        let position = tree
            ...
            .map_err(|e| Error::Shielder(format!("Orchard position lookup failed: {e:?}")))?
            .ok_or(Error::UnknownCommitment)?;
        let merkle_path = tree
            .witness_at_checkpoint_id(position, &0)
            .map_err(|e| Error::Shielder(format!("Orchard witness lookup failed: {e:?}")))?
            .ok_or(Error::UnknownCommitment)?;
...
fn verify_bundle(
    ...
    bundle
        .verify_proof(vk)
        .map_err(|e| Error::Shielder(format!("Orchard proof verification failed: {e:?}")))?;
