...
- **Go Refactoring**: When renaming types, functions, or variables in Go code, always use `gorename` instead of text-based find/replace. Example: `gorename -from '"github.com/thornadocash/go-thornado/x/thorchain".TypeName' -to NewTypeName`. This tool understands Go's type system and scope, avoiding false matches in comments/strings and handling shadowed variables correctly.
