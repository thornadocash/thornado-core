...
func (ts *tssSignableBTC) GetPubKey() *btcec.PublicKey {
	...
	if err != nil {
		ts.log.Err(err).Str("pubkey", ts.poolPubKey.String()).Msg("fail to get public key from the bech32 pool public key string")
		return nil
	}
	secpPubKey, err := codec.ToCmtPubKeyInterface(cpk)
	if err != nil {
		ts.log.Err(err).Msgf("%s is not a secp256k1 public key", ts.poolPubKey)
		return nil
	}
