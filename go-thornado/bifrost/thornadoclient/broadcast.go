...
func (b *thornadoBridge) broadcast(mode string, msgs ...stypes.Msg) (common.TxID, error) {
	...
	if blockHeight > b.blockHeight {
		...
		if err != nil {
			return noTxID, fmt.Errorf("fail to get account number and sequence number from thornado : %w", err)
		}
	...
	flags := flag.NewFlagSet("thornado", 0)
	...
	if commit.Code > 0 {
		...
		// commit code 6 means `unknown request` , which means the tx can't be accepted by thornado
	...
	b.logger.Info().Msgf("Received a TxHash of %v from the thornado", commit.TxHash)

	return txHash, nil
}

// Broadcast Broadcasts tx to thornado using sync mode (waits for CheckTx)
func (b *thornadoBridge) Broadcast(msgs ...stypes.Msg) (common.TxID, error) {
	return b.broadcast("sync", msgs...)
}

// BroadcastWithBlocking broadcasts tx to thornado and waits for block commitment
func (b *thornadoBridge) BroadcastWithBlocking(msgs ...stypes.Msg) (common.TxID, error) {
