...
fn path_tweak_root(
    ...
    path_index: u64,
) -> Result<Option<[u8; 32]>, WrapperError> {
    if path_index == 0 {
        return Ok(None);
    }
    let pubkey =
        hex::decode(public_key_compressed).map_err(|e| WrapperError::Message(e.to_string()))?;
    let mut hasher = Sha256::new();
    hasher.update(b"thornado:vault-path:v1");
    hasher.update(pubkey);
    hasher.update(path_index.to_be_bytes());
    Ok(Some(hasher.finalize().into()))
}

fn tweaked_public_key_package(
    public_key_package: &PublicKeyPackage,
    public_key_compressed: &str,
    path_index: u64,
) -> Result<PublicKeyPackage, WrapperError> {
    match path_tweak_root(public_key_compressed, path_index)? {
        Some(root) => Ok(public_key_package.clone().tweak(Some(root))),
        None => Ok(public_key_package.clone()),
    }
}
...
fn write_json<T: Serialize>(value: &T, out: *mut GoSchnorrBuf) -> i32 {
    if out.is_null() {
        return -1;
    }
...
fn write_bytes(mut bytes: Vec<u8>, out: *mut GoSchnorrBuf) -> i32 {
    if out.is_null() {
        return -1;
    }
    let buf = GoSchnorrBuf {
...
fn write_error(err: WrapperError, out: *mut GoSchnorrBuf) -> i32 {
    let ffi = match err {
...
fn sign_session_new(input: SessionInput) -> Result<SessionKind, WrapperError> {
    ...
    if message.len() != 32 {
        return Err(WrapperError::Message(format!(
            "schnorr messages must be 32 bytes, got {}",
            message.len()
        )));
    }
    let path_index = input.path_index.unwrap_or(0);
    ...
    Ok(SessionKind::Sign(SignSession {
        ...
        message,
        path_index,
        public_key_compressed: stored.public_key_compressed,
        key_package,
        public_key_package,
        nonces: Some(nonces),
        ...
        match msg.kind.as_str() {
            "sign_round1" => {
                    ...
                    && self.signature_shares.is_empty()
                {
                    ...
                    let tweak_root = path_tweak_root(&self.public_key_compressed, self.path_index)?;
                    let share = match tweak_root.as_ref() {
            ...
            "sign_round2" => {
                ...
                if self.signature_shares.len() == self.participants.len() && self.result.is_none() {
                    let signing_package =
                        SigningPackage::new(self.commitments.clone(), &self.message);
                    let tweak_root = path_tweak_root(&self.public_key_compressed, self.path_index)?;
                    let signature = match tweak_root.as_ref() {
...
fn sign(input: SignInput) -> Result<SignOutput, WrapperError> {
        ...
        != 32
    {
        return Err(WrapperError::Message(
            "schnorr messages must be 32 bytes".to_string(),
        ));
    }
    let message = B64
        .decode(&input.message)
    ...
    if share.all_key_packages.is_empty() {
        return Err(WrapperError::Message(
            "one-shot signing is unavailable for distributed schnorr keyshares".to_string(),
        ));
    }
    let public_key_package = PublicKeyPackage::deserialize(
    ...
    for (id, key_package) in key_packages.iter() {
        ...
        let tweak_root = path_tweak_root(&share.public_key_compressed, path_index)?;
        let sig_share = match tweak_root.as_ref() {
            Some(root) => frost::round2::sign_with_tweak(
                &signing_package,
                nonces,
                key_package,
...
fn with_session<T>(
    ...
    let session = sessions
        ...
        .ok_or_else(|| WrapperError::Message(format!("invalid schnorr session handle {handle}")))?;
    f(session)
}

#[no_mangle]
pub extern "C" fn goschnorr_keygen(ptr: *const u8, len: usize, out: *mut GoSchnorrBuf) -> i32 {
...
#[no_mangle]
pub extern "C" fn goschnorr_sign(ptr: *const u8, len: usize, out: *mut GoSchnorrBuf) -> i32 {
...
#[no_mangle]
pub extern "C" fn goschnorr_verify(ptr: *const u8, len: usize, out: *mut GoSchnorrBuf) -> i32 {
...
#[no_mangle]
pub extern "C" fn goschnorr_keygen_session_new(
    ptr: *const u8,
    len: usize,
    handle_out: *mut i32,
    err_out: *mut GoSchnorrBuf,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn goschnorr_sign_session_new(
    ptr: *const u8,
    len: usize,
    handle_out: *mut i32,
    err_out: *mut GoSchnorrBuf,
...
#[no_mangle]
pub extern "C" fn goschnorr_session_output_message(handle: i32, out: *mut GoSchnorrBuf) -> i32 {
...
#[no_mangle]
pub extern "C" fn goschnorr_session_message_receiver(
    _handle: i32,
    ptr: *const u8,
    len: usize,
    index: u32,
    out: *mut GoSchnorrBuf,
) -> i32 {
    let result = (|| {
        if ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn goschnorr_session_input_message(
    handle: i32,
    ptr: *const u8,
    len: usize,
...
#[no_mangle]
pub extern "C" fn goschnorr_session_finish(handle: i32, out: *mut GoSchnorrBuf) -> i32 {
    match with_session(handle, |session| match session {
        SessionKind::Keygen(s) => s.result.clone().ok_or_else(|| {
            WrapperError::Message("schnorr keygen session is not finished".to_string())
        }),
        SessionKind::Sign(s) => s.result.clone().ok_or_else(|| {
            WrapperError::Message("schnorr sign session is not finished".to_string())
        }),
    }) {
        Ok(bytes) => write_bytes(bytes, out),
        Err(err) => write_error(err, out),
...
#[no_mangle]
pub extern "C" fn goschnorr_session_abort_message(handle: i32, out: *mut GoSchnorrBuf) -> i32 {
    let result = with_session(handle, |session| {
...
#[no_mangle]
pub extern "C" fn goschnorr_session_free(handle: i32) -> i32 {
    let mut sessions = SESSIONS.lock().expect("session lock poisoned");
...
#[no_mangle]
pub extern "C" fn goschnorr_free(ptr: *mut c_void, len: usize) {
