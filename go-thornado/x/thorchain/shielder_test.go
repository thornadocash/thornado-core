...
func TestBondSplitCommitmentsAreProtocolGenerated(t *testing.T) {
	...
	deposit := types.ShielderDeposit{
		...
		ValidatorPubKey: "thornadovalconspub1addwnpepqwnn92m9kz5mt6q690sy3z9le56d459k5m9twe5k9uc06z6j337ssxq92g5",
		NodeSlot:        3,
	}
...
func TestWithdrawalFeeIsProtocolTwoPercent(t *testing.T) {
	if got := shielderWithdrawalFeeSats(100_000_000); got != 2_000_000 {
...
func TestFeeClaimPayloadBindsRecipientAndCounter(t *testing.T) {
	...
	payload := shielderFeeClaimPayload("validator", owner, 100, 7, notes, notePubKeys)
	if string(payload) == string(shielderFeeClaimPayload("validator", owner, 101, 7, notes, notePubKeys)) {
		t.Fatal("payload did not bind accrued amount")
	}
	if string(payload) == string(shielderFeeClaimPayload("validator", owner, 100, 8, notes, notePubKeys)) {
		t.Fatal("payload did not bind fee share counter")
	}
	otherNotePubKeys := []common.PubKey{GetRandomPubKey()}
	if string(payload) == string(shielderFeeClaimPayload("validator", owner, 100, 7, notes, otherNotePubKeys)) {
