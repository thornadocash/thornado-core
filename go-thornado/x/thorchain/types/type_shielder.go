...
func (m ShielderDeposit) IsValidatorBond() bool {
	return strings.TrimSpace(m.ValidatorPubKey) != ""
}
...
type ShielderValidatorBond struct {
...
type NodeSlotAuction struct {
	...
	SellerValidatorPubKey string            `json:"seller_validator_pub_key"`
	Slot                  uint64            `json:"slot"`
...
func (m NodeSlotAuction) Valid() error {
	...
	if strings.TrimSpace(m.SellerValidatorPubKey) == "" {
		return fmt.Errorf("missing node slot auction seller validator pubkey")
	}
...
func (m NodeSlotBid) Valid() error {
	...
	if strings.TrimSpace(m.ValidatorPubKey) == "" {
		return fmt.Errorf("missing node slot bid validator pubkey")
	}
	return nil
}

func (m ShielderValidatorBond) Key() string {
	return strings.TrimSpace(m.ValidatorPubKey)
}

func (m ShielderValidatorBond) Valid() error {
	if strings.TrimSpace(m.ValidatorPubKey) == "" {
		return fmt.Errorf("missing shielder validator pubkey")
	}
	if m.OperatorPubKey.IsEmpty() {
		return fmt.Errorf("missing shielder validator operator pubkey")
	}
	if m.NodeAddress.Empty() {
		return fmt.Errorf("missing shielder validator node address")
	}
	return nil
}
