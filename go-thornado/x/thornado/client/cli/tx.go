...
// GetCmdProposeUpgrade command to propose an upgrade as a node
func GetCmdProposeUpgrade() *cobra.Command {
	const flagInfo = "info"

	cmd := &cobra.Command{
		Use:   "propose-upgrade [name] [height]",
		Short: "propose an upgrade for vote by other active nodes",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
// GetCmdApproveUpgrade command to approve an upgrade as a node
func GetCmdApproveUpgrade() *cobra.Command {
	return &cobra.Command{
		Use:   "approve-upgrade [name]",
		Short: "approve an upgrade that has been proposed for vote by another active node",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
// GetCmdRejectUpgrade command to reject an upgrade as a node
func GetCmdRejectUpgrade() *cobra.Command {
	return &cobra.Command{
		Use:   "reject-upgrade [name]",
		Short: "reject an upgrade that has been proposed for vote by another active node",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func GetCmdSetNodeKeys() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgSetNodeKeys(pk, nodeConsPubKeyStr, clientCtx.GetFromAddress())
