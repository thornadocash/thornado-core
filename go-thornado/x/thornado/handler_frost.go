...
type FrostHandler struct {
	mgr Manager
}

// NewFrostHandler create a new handler to process MsgKeygenVault
func NewFrostHandler(mgr Manager) FrostHandler {
	return FrostHandler{mgr: mgr}
}

// Run it the main entry point to execute Version logic
func (h FrostHandler) Run(ctx cosmos.Context, m cosmos.Msg) (*cosmos.Result, error) {
...
func (h FrostHandler) validate(ctx cosmos.Context, msg *MsgKeygenVault) error {
	...
	if msg.KeygenType != BaseVaultKeygen {
		return fmt.Errorf("only base vaults allowed for frost")
	}
	if msg.IsSuccess() {
	...
	for _, keygen := range keygenBlock.Keygens {
		...
		for _, member := range keygen.GetMembers() {
			...
			if err == nil && addr.Equals(msg.Signer) {
				return validateFrostAuth(ctx, h.mgr.Keeper(), msg.Signer)
			}
		}
	}
...
func validateFrostAuth(ctx cosmos.Context, k keeper.Keeper, signer cosmos.AccAddress) error {
...
func (h FrostHandler) handle(ctx cosmos.Context, msg *MsgKeygenVault) error {
	ctx.Logger().Info("handler frost", "current version", h.mgr.GetVersion())
	...
	if len(msg.Blame) > 0 {
		...
		for _, b := range msg.Blame {
			for _, bn := range b.BlameNodes {
				...
				if err != nil {
					ctx.Logger().Error("fail to get frost keygen pubkey", "pubkey", bn.Pubkey, "error", err)
					continue
				}
				acc, err := pk.GetThorAddress()
				if err != nil {
					ctx.Logger().Error("fail to get frost keygen thor address", "pubkey", bn.Pubkey, "error", err)
					continue
				}
	...
	// only record FROST metric when keygen is success
	if msg.IsSuccess() && !msg.VaultPubKey.IsEmpty() {
		metric, err := h.mgr.Keeper().GetFrostKeygenMetric(ctx, msg.VaultPubKey)
		if err != nil {
			ctx.Logger().Error("fail to get keygen metric", "error", err)
		} else {
			ctx.Logger().Info("save keygen metric to db")
			metric.AddNodeFrostTime(msg.Signer, msg.KeygenTime)
			h.mgr.Keeper().SetFrostKeygenMetric(ctx, metric)
		}
	}
	voter, err := h.mgr.Keeper().GetFrostVoter(ctx, msg.ID)
	if err != nil {
		return fmt.Errorf("fail to get frost voter: %w", err)
	}

	// when VaultPubKey is empty , which means FrostVoter with id(msg.ID) doesn't
	...
	h.mgr.Keeper().SetFrostVoter(ctx, voter)
	...
	if voter.BlockHeight == 0 {
		...
		if !msg.IsSuccess() {
			...
			for _, b := range msg.Blame {
				...
				if err := h.mgr.EventMgr().EmitEvent(ctx, NewEventFrostKeygenFailure(b.FailReason, b.Round, b.IsUnicast, msg.Height, blames)); err != nil {
	...
	if msg.IsSuccess() {
		...
		if voter.HasCompleteConsensus() {
			...
			var metric *keeper.FrostKeygenMetric
			metric, err = h.mgr.Keeper().GetFrostKeygenMetric(ctx, msg.VaultPubKey)
			if err != nil {
				ctx.Logger().Error("fail to get keygen metric", "error", err)
			} else {
				var total int64
				for _, item := range metric.NodeFrostTimes {
					total += item.FrostTime
				}
				evt := NewEventFrostKeygenMetric(metric.PubKey, metric.GetMedianTime())
				if err = h.mgr.EventMgr().EmitEvent(ctx, evt); err != nil {
					ctx.Logger().Error("fail to emit frost metric event", "error", err)
				}
			}

			if len(initVaults) == len(keygenBlock.Keygens) {
				ctx.Logger().Info("frost keygen results churn", "baseVaults", len(initVaults))
			...
			if err != nil {
				...
				if err := h.mgr.EventMgr().EmitEvent(ctx, NewEventFrostKeygenSuccess(msg.VaultPubKey, msg.Height, members)); err != nil {
...
func judgeLateSigner(ctx cosmos.Context, mgr Manager, msg *MsgKeygenVault, voter FrostVoter) {
	...
	mgr.Keeper().SetFrostVoter(ctx, voter)
	for _, member := range msg.PubKeys {
		pkey, err := common.NewPubKey(member)
		if err != nil {
			ctx.Logger().Error("fail to get pub key", "error", err)
...
func FrostAnteHandler(ctx cosmos.Context, v semver.Version, k keeper.Keeper, msg MsgKeygenVault) (cosmos.Context, error) {
	...
	if err := validateFrostAuth(ctx, k, msg.Signer); err != nil {
		return ctx, err
	}
	voter, err := k.GetFrostVoter(ctx, msg.ID)
	if err != nil {
		return ctx, err
	}
	if voter.IsEmpty() {
		voter = NewFrostVoter(msg.ID, msg.PubKeys, msg.VaultPubKey, msg.VaultPubKeyEddsa)
	}
	if !voter.Sign(msg.Signer, msg.Chains, string(msg.Secp256K1Signature)) {
