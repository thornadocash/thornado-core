...
func scheduleUpgradeIfNecessary(ctx cosmos.Context, k keeper.Keeper, name string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to determine if upgrade is approved by majority threshold of nodes: %w", err)
	}
...
// ActiveNodeAnteHandler called by the ante handler to gate mempool entry and
