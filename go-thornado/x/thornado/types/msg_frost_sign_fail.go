...
var (
	_ sdk.Msg              = &MsgFrostKeysignFail{}
	_ sdk.HasValidateBasic = &MsgFrostKeysignFail{}
	_ sdk.LegacyMsg        = &MsgFrostKeysignFail{}
)

// NewMsgFrostKeysignFail create a new instance of MsgFrostKeysignFail message
func NewMsgFrostKeysignFail(height int64, blame Blame, coins common.Coins, signer cosmos.AccAddress, pubKey common.PubKey) (*MsgFrostKeysignFail, error) {
	// Normalize the round before hashing so outgoing messages always use the
	// canonical wire format and stable consensus ID.
	round, err := NormalizeFrostKeysignRound(blame.Round)
	if err != nil {
		return nil, fmt.Errorf("fail to normalize keysign fail round: %w", err)
	}
	blame.Round = round
	id, err := getMsgFrostKeysignFailID(blame.BlameNodes, height, coins, pubKey, blame.Round)
	if err != nil {
		return nil, fmt.Errorf("fail to get keysign fail id:%w", err)
	}
	return &MsgFrostKeysignFail{
		ID:     id,
		Height: height,
		Blame:  blame,
		Coins:  coins,
...
// getFrostKeysignFailID this method will use all the members that caused the frost
...
func getMsgFrostKeysignFailID(members []Node, height int64, coins common.Coins, pubKey common.PubKey, round string) (string, error) {
...
func (m *MsgFrostKeysignFail) ValidateBasic() error {
	if m.Signer.Empty() {
	...
	if m.Blame.IsEmpty() {
		return cosmos.ErrUnknownRequest("frost blame is empty")
	}
	if len(m.Blame.FailReason) > MaxFrostTextLength {
		return cosmos.ErrUnknownRequest("frost blame reason too long")
	}
	if len(m.Blame.Round) > MaxFrostTextLength {
		return cosmos.ErrUnknownRequest("frost blame round too long")
	}
	if len(m.Blame.BlameNodes) > MaxFrostBlames {
		return cosmos.ErrUnknownRequest("too many frost blame nodes")
	}
	...
	round, err := NormalizeFrostKeysignRound(m.Blame.Round)
	if err != nil {
...
func (m *MsgFrostKeysignFail) GetSigners() []cosmos.AccAddress {
	return []cosmos.AccAddress{m.Signer}
}

func MsgFrostKeysignFailCustomGetSigners(m proto.Message) ([][]byte, error) {
	msg, ok := m.(*types.MsgFrostKeysignFail)
	if !ok {
		return nil, fmt.Errorf("can't cast as MsgFrostKeysignFail: %T", m)
	}
