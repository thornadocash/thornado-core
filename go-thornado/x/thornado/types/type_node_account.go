...
func NewNodeAccount(nodeAddress cosmos.AccAddress, status NodeStatus, nodePubKeySet common.PubKeySet, nodeConsPubKey string, bond cosmos.Uint, bondAddress common.Address, height int64) NodeAccount {
	na := NodeAccount{
		NodeAddress:    nodeAddress,
		PubKeySet:      nodePubKeySet,
		NodeConsPubKey: nodeConsPubKey,
		Bond:           bond,
...
func (m *NodeAccount) Equals(n1 NodeAccount) bool {
	if m.NodeAddress.Equals(n1.NodeAddress) &&
		...
		m.NodeConsPubKey == n1.NodeConsPubKey &&
...
func (m *NodeAccount) String() string {
	...
	sb.WriteString("node consensus pub key:" + m.NodeConsPubKey + "\n")
...
// IsNodeKeys validate whether the given account address belongs to an currently active node
